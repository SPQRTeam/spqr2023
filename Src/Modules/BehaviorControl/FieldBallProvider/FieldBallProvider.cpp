...
void FieldBallProvider::update(FieldBall& fieldBall)
{
  ...
  checkIfBallIsInsideDefenderCircle(fieldBall);
}
...
{
  fieldBall.isInsideGoalieCircle = false;
  fieldBall.isInsideDefenderCircle = false;
  fieldBall.isInsideLeftDefenderCircle = false;
  fieldBall.isInsideRightDefenderCircle = false;

  bool ballSeen = fieldBall.ballWasSeen();
  bool ballValid = theTeamBallModel.isValid;
  if(!ballSeen && !ballValid)
    return;

  const Vector2f goalCenter{theFieldDimensions.xPosOwnGroundLine, 0.f};
  const Vector2f ballPos = ballSeen ? fieldBall.recentBallPositionOnField() : theTeamBallModel.position; //global
  const Vector2f endBallPos = fieldBall.teamEndPositionOnField;

  fieldBall.isInsideGoalieCircle = (ballPos - goalCenter).norm() < 750 || (endBallPos - goalCenter).norm() < 750;

  bool goaliePlaying = false;
  if (thePlayerRole.isGoalkeeper()) {
    goaliePlaying = true;
  }
  else {
    for (Teammate mate : theTeamData.teammates) {
      if (mate.isGoalkeeper && mate.status == Teammate::Status::PLAYING) {
        goaliePlaying = true;
      }
    }
  }

  fieldBall.isInsideDefenderCircle = (ballPos - goalCenter).norm() < 1500;
  if (goaliePlaying) {
    fieldBall.isInsideDefenderCircle = fieldBall.isInsideDefenderCircle && !fieldBall.isInsideGoalieCircle;
  }

  fieldBall.isInsideLeftDefenderCircle = fieldBall.isInsideDefenderCircle && ballPos.y() >= 0;
  fieldBall.isInsideRightDefenderCircle = fieldBall.isInsideDefenderCircle && ballPos.y() < 0;
}
