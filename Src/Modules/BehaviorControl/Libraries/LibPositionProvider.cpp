...
Pose2f LibPositionProvider::myReadyPosition() const {
  ...
  Pose2f supporterPose{}; //3
  Pose2f receiverPose{}; //7
  Pose2f liberoPose{}; //6
  
  if(theGameInfo.kickingTeam == theOwnTeamInfo.teamNumber){ //kickoff di attacco
    strikerPose = Pose2f(0.f, -500.f, 0.f);
    supporterPose = Pose2f(0.f, -1500.f, 1500.f);
    receiverPose = Pose2f(0.f, -800.f, -1500.f);
    liberoPose = Pose2f(0.f, -2000.f, 0); 
  }else{ //kickoff di difesa
  ...
  else
  {
    ...
    std::vector<int> readyPlayerNums{4, 2, 5, 3, 7, 6}; //ordered by priority
    ...
    for(auto i : readyPlayerNums){
      ...
      if(i == theRobotInfo.number || !isPenalized)
