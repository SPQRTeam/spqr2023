...
private:
  ...
  */
  int calcCornerZone(const Vector2f& point, char side) const;
  ...
  */
  ...
  /**
   * Checks if the corridor (practically a 2D cylinder) is covered an object, which can be a teammate or an opponent
  */
  bool isCorridorCovered(const Vector2f start, const Vector2f end, const float width, const Vector2f object) const;
