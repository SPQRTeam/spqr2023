/**
* @file FreeKickDefenderoneCard.cpp
* @author @neverorfrog
*/
...
class FreeKickDefenderoneCard : public FreeKickDefenderoneCardBase
{
  ...
  option
  {
    ...
    state(opponentKickIn)
    {
      ...
      action
      {
        //Returns a position for the defenderone in a defensive kick in configuration
        Vector2f defenderonePos = theLibSpec.getPositionCoveringDangerousOpponent(false);
        Pose2f position = theLibMisc.glob2Rel(defenderonePos.x(), defenderonePos.y());
...
MAKE_CARD(FreeKickDefenderoneCard);