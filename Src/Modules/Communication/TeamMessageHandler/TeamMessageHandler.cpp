...
bool TeamMessageHandler::isDifferent(){
  ...
  bool time_condition = (timeLastSent < theFrameInfo.time);
...
void TeamMessageHandler::update(BHumanMessageOutputGenerator& outputGenerator){
  ...
  bool sendDueToConditionalEvent = theMessageManagement.lastEventTS > timeLastSent;
  ...
  outputGenerator.sendThisFrame =
    ...
    (
      (         // periodic
        theFrameInfo.getTimeSince(timeLastSent) >= theMessageManagement.sendInterval
        #if USE_SIMILARITY_FUNCTION
        && isDifferent()    // TODO: check after a test match if the sent packages are fine or they are too few 
        #endif
      ) ||
      theFrameInfo.time < timeLastSent ||    // for the first packet
      #if USE_SIMILARITY_FUNCTION
      theFrameInfo.getTimeSince(timeLastSent) > 30000 ||   // make sure messages are sent every once in a while, to look alive in the net
      #endif
  ...
  // if(sendDueToConditionalEvent == true || isDifferent() || theFrameInfo.getTimeSince(timeLastSent) > 30000){
...
void TeamMessageHandler::writeMessage(BHumanMessageOutputGenerator& outputGenerator, RoboCup::SPLStandardMessage* const m, bool sendDueToConditionalEvent) const{ 
  ...
  timeLastSent = theFrameInfo.time;    // SPQR simplified this, reasons for BHuman to do the other way are unclear.
